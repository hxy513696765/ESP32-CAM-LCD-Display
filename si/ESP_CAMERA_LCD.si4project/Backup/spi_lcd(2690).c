...
{	/*
    ...
    */
    PIN_FUNC_SELECT(PERIPHS_IO_MUX_MTDO_U,2);   //DC PIN
    PIN_FUNC_SELECT(PERIPHS_IO_MUX_MTCK_U,2);   //RESET PIN
    PIN_FUNC_SELECT(PERIPHS_IO_MUX_GPIO5_U,2);    //BKL PIN
    WRITE_PERI_REG(GPIO_ENABLE_W1TS_REG, BIT15|BIT13|BIT5);
}
...
static void spi_master_init()
{
    ...
    */
    ...
    WRITE_PERI_REG(GPIO_ENABLE_W1TS_REG, BIT4|BIT14|BIT2|BIT12);
...
void ili9341_display_frame(const uint16_t xs, const uint16_t ys, const uint16_t width, const uint16_t height,uint16_t * data[]){
    int x, y;
    int i;
    uint16_t x1, y1;
    uint32_t xv, yv, dc;
    ...
    for (y=0; y<height; y++) {
        ...
        while (x<width) {
            for (i=0; i<16; i++) {
                ...
                //x1 = data[0];x++;//myPalette[(unsigned char)(data[y][x])]; x++;
                //y1 = data[1];x++;//myPalette[(unsigned char)(data[y][x])]; x++;
                x1 = data[y][x];x++;//myPalette[(unsigned char)(data[y][x])]; x++;
                y1 = data[y][x];x++;//myPalette[(unsigned char)(data[y][x])]; x++;
                temp[i] = U16x2toU32(x1,y1);
            }
            while (READ_PERI_REG(SPI_CMD_REG(SPI_NUM))&SPI_USR);
            for (i=0; i<16; i++) {
            	WRITE_PERI_REG((SPI_W0_REG(SPI_NUM) + (i << 2)), 0xFFFF);
                //WRITE_PERI_REG((SPI_W0_REG(SPI_NUM) + (i << 2)), temp[i]);
                //WRITE_PERI_REG((SPI_W0_REG(SPI_NUM) + (i << 2)), temp[i]);
            }
            SET_PERI_REG_MASK(SPI_CMD_REG(SPI_NUM), SPI_USR);
...
void displayBmpPicture(char * file){
	FILE* fp;
	esp_err_t ret;
	fp = fopen(file, "r");
	if (fp == NULL) {
		printf("BMP File not found [%s]\r\n", file);
		return;
	}
	else{
		printf("BMP File is [%s]\r\n", file);
	}
	...
	uint16_t row = 0;
	uint16_t col = 0;
	unsigned int r888Color;
	fseek(fp,54+(col*3)+(count),0);
	for(row = 0;row < maxRow;row++){
		...
		for(col = 0;col < maxCol;col++){
			...
			fread(buf, 3, 1, fp);
			r888Color = buf[2] << 16;
			r888Color = r888Color | (buf[1] << 8);
			r888Color = r888Color | buf[0];
			displayBuffer[row][col] = RGB888ToRGB565(r888Color);
...
void reFreashLcdBuffer(char * file,uint16_t pictureStart_X,uint16_t pictureStart_Y,uint16_t pictureWith,uint16_t pictureHeight,uint16_t lcdStart_X,uint16_t lcdStart_Y){
	FILE* fp;
	esp_err_t ret;
	fp = fopen(file, "r");
	if (fp == NULL) {
	...
	uint16_t row = 0;
	uint16_t col = 0;
	uint16_t lcdBufferRow = 0;
	uint16_t lcdBufferCol = 0;
	unsigned int r888Color;
	fseek(fp,54+(col*3)+(count),0);
	for(row = pictureStart_X;row < pictureStart_X + pictureWith;row++){
		//fseek(fp,54+(col*3)+(count),0);
		for(col = pictureStart_Y;col < pictureStart_Y + pictureHeight;col++){
			fseek(fp,54+(col*3)+(count),0);			
			fread(buf, 3, 1, fp);
			r888Color = buf[2] << 16;
			r888Color = r888Color | (buf[1] << 8);
			r888Color = r888Color | buf[0];
			displayBuffer[lcdStart_X + lcdBufferRow][lcdStart_Y + lcdBufferCol] = RGB888ToRGB565(r888Color);
			//printf("%02x%02x%02x ",buf[0],buf[1],buf[2]);
			lcdBufferCol++;
		}
		lcdBufferRow++;
...
void lcdDisplay(char * file,uint16_t start_X,uint16_t start_Y,uint16_t display_With,uint16_t display_Height){
	FILE* fp;
	esp_err_t ret;
	fp = fopen(file, "r");
	if (fp == NULL) {
	...
	unsigned int r888Color;
	fseek(fp,54+(col*3)+(count),0);
	for(row = start_X;row < start_X + display_With;row++){
		//fseek(fp,54+(col*3)+(count),0);
		for(col = start_Y;col < start_Y + display_Height;col++){
			fseek(fp,54+(col*3)+(count),0);			
			fread(buf, 3, 1, fp);
			r888Color = buf[2] << 16;
			r888Color = r888Color | (buf[1] << 8);
...
void ili9341_init()
{
	...
	displayBuffer = (uint16_t**)malloc(sizeof(uint16_t*)*240);
	
	int row,col;
	for(row = 0;row < 240;row++){
		//displayBuffer[row] = (uint16_t *)malloc(sizeof(uint16_t) * 320);
		displayBuffer[row] = (uint16_t *)malloc(sizeof(uint16_t) * 320);
	}
	/*
	*/
	//memset(displayBuffer,0x000F, 320*240); 
	/**/
	//displayBuffer = (uint16_t *)malloc(sizeof(uint16_t) * 320 * 240);
    spi_master_init();
    ili_gpio_init();
    ILI9341_INITIAL ();
}
